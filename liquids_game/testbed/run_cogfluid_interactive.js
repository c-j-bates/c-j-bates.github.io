...
function NewScene(value) {
  // debugger;
  scene.remove(window.fill_line);
...
function testSwitchAndNewScene(testName) {
  generateSceneGeometry();
  testSwitch(testName);
}
...
function Testbed(obj) {
  // Init world
  //Init
  window.scene_type = "cogfluid_interactive";
  var that = this;
  ...
  generateSceneGeometry();
  testSwitch("cogfluid_interactive");
  render(paused);
}
...
function toggleViscosity() {
  if (window.initiated === false) {
    ...
    else if (window.particleFlags === b2_viscousParticle) {
      window.particleFlags = b2_waterParticle;
      elem.value = 'High viscosity';
      ResetScene();
    }
    else if (window.particleFlags === undefined) {
      window.particleFlags = b2_viscousParticle;
      elem.value = 'Low viscosity';
      ResetScene();
    }
